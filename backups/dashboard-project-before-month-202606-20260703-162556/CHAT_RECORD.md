...
此文件最初記錄的是專案早期拆分狀態。後續專案已繼續演進，目前重要狀態如下：
...
2. 目前 `defaultMonth` 是 `202605`。

3. `202605` 是 current schema，`202604` 是 legacy schema。

4. `app.js` 已改為 ES module，並抽出 helper：
...
5. PDF 匯出已加入，且改成 manifest-driven：
...
6. 一般 dashboard 與 print layout 已有隔離檢查：
...
7. 目前沒有資料庫層，仍是 portable JSON dashboard。是否升級 SQLite / API / PostgreSQL，應看跨月份查詢、多使用者、權限、資料量與近實時需求是否出現。

8. 後續若要測試 PDF 壓力，可用虛擬月份或 fixture，不要直接污染正式資料。
