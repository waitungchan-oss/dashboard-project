...
除非使用者明確要求暫停或只做草稿，Codex 不可在完成正式修改後跳過 Obsidian 回填。回填至少要包含本文件第 11 節列出的欄位，並記錄 Hermes 結果與 commit hash。若修改導致回歸、驗收失敗或系統邊界改變，必須同步建立或更新 ADR / Incident。
...
Obsidian 回填是正式修改完成條件之一，不是可選附註。未完成回填時，不得宣稱正式修改已完整收尾；若因使用者要求或外部狀態暫時無法回填，必須在最終回覆中明確列出缺口與待補事項。
