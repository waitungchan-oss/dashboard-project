...
const DashboardApp = (function() {
    ...
    return {
        ...
        initCharts: function() {
            ...
            initChartSafe('npsCorrelationChart', { type: 'scatter', data: { datasets: [{ label: '服務環節', data: npsPoints, backgroundColor: (ctx) => { const val = ctx.raw; if (!val) return '#ccc'; if (val.x > npsCorrelationData.threshold.x && val.y < npsCorrelationData.threshold.y) return pbi8; if (val.x > npsCorrelationData.threshold.x && val.y >= npsCorrelationData.threshold.y) return pbi4; if (val.x <= npsCorrelationData.threshold.x && val.y >= npsCorrelationData.threshold.y) return pbi1; return softGrey; }, pointRadius: 8, pointHoverRadius: 10 }] }, options: { responsive: true, maintainAspectRatio: false, layout: { padding: { top: 25, right: 30, left: 15, bottom: 10 } }, plugins: { legend: { display: false }, quadrantCrosshair: true, datalabels: { display: true, align: 'top', anchor: 'end', offset: 4, color: '#252423', font: { weight: 'bold', size: 11 }, formatter: (val) => val.item }, tooltip: { callbacks: { label: (ctx) => `${ctx.raw.item}: 滿意度 ${ctx.raw.y.toFixed(2)}, 對總分相關性 ${ctx.raw.x.toFixed(3)}, 對推薦意願相關性 ${(ctx.raw.recommendationCorrelation ?? 0).toFixed(3)}` } } }, scales: { x: { title: { display: true, text: '對總分影響力 / 相關性 (Importance)', font: { weight: 'bold' } }, min: npsXMin, max: npsXMax }, y: { title: { display: true, text: npsScaleLabel, font: { weight: 'bold' } }, min: npsYMin, max: npsYMax } } } });
